import random
from src.time_helper import convert_date
from datetime import datetime
from discord.ext import commands
from discord import Embed
from cogs.daily_leaderboard import download_leaderboard
import os
...
Challenges = [['go ' + x for x in Route], 'kill all shopkeepers', 'remove your HUD',
            ...
            ['curse yourself on ' + level + ' and cant uncurse' for level in Levels], 'rescue the damsel every level', 'sacrifice as many bodies as possible', 
...
def createChallenge():
    ...
    if randomEnding == "Normal":
        ChallengesPool = [x for x in ChallengesPool if x not in [['curse yourself on ' + level + ' and cant uncurse' for level in Levels], 
                                                                ['bring a ' + item + ' to end of run' for item in Items]]]
    ...
    if numOfChallenges > 1:
        ChallengesPool = [x for x in ChallengesPool if x not in ['pick up no gold', 'kill everyone', 'kill as little as possible', 'play a low% run']]
...
class DailyStruggle(commands.Cog):
    ...
    async def dailystruggle(self, ctx):
        ...
        embed = Embed(description=data, color=0xD2691E, title='The Daily Struggle: ' + datetime.utcnow().strftime('%m-%d-%Y'))
